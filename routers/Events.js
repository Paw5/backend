...
export const prepareQuery = (fields, limit, page, filterParams) => {
  ...
  const query = `SELECT ${sqlFields} FROM events ${whereQuery} ${limitSql}`; // assemble the query together
  ...
  if (!Number(userId)) { // if a not a number return error
    ...
    const filteredBody = Object
      .fromEntries(
        Object
          .entries(req.body)
          .filter(([key]) => (key === 'name') || (key === 'time')),
      );
    filteredBody.user_id = userId;
    // create a new record in table
    connection.query(query, filteredBody).then(() => {
      connection.query('SELECT * FROM events WHERE event_id=last_insert_id()').then(([rows]) => { // return new addition
