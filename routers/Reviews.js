...
router.post('/:locationId', (req, res) => {
  ...
  const { locationId } = req.params; // pull out user id
  if (!Number(locationId)) { // if a not a number return error
    res.sendStatus(400);
    ...
    const filteredBody = Object
      .fromEntries(
        Object
          .entries(req.body)
          .filter(([key]) => (key === 'user_id') || (key === 'description') || (key === 'time') || (key === 'pet_rating') || (key === 'star_rating')),
      );
    filteredBody.location_id = locationId;
    // create a new record in table
    connection.query(query, filteredBody).then(() => {
      connection.query('SELECT * FROM reviews WHERE review_id=last_insert_id()').then(([rows]) => { // return new addition
