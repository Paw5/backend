...
export const prepareQuery = (fields, limit, page, filterParams) => {
  ...
  const query = `SELECT ${sqlFields} FROM tags ${whereQuery} ${limitSql}`; // assemble the query together
...
router.post('/:postId', (req, res) => {
  ...
  const { postId } = req.params; // pull out user id
  if (!Number(postId)) { // if a not a number return error
    res.sendStatus(400);
    ...
    filteredBody.post_Id = postId;
    // create a new record in table
    connection.query(query, filteredBody).then(() => {
      connection.query('SELECT * FROM tags WHERE tag_id=last_insert_id()').then(([rows]) => { // return new addition
